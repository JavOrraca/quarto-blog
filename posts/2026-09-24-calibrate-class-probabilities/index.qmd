---
title: "Calibrate Class Probabilities in the Workflow"
...
The rows are a simulated set from modeldata. The folded chunk is the split. The workflow in the next section is the part worth copying.
...
`min_n = 2` and a deep tree are deliberate. The leaves go pure, so `rpart` reports class probabilities of 0 and 1. That is the failure mode I want on the page. A tuned depth comes later.
...
Accuracy is `r tree_scores$accuracy[1]` either way, against a majority-class baseline of `r round(majority_accuracy, 3)`. ROC AUC stays at `r tree_scores$roc_auc[1]`. The hard class matches on `r sum(tree_before$predictions$.pred_class == tree_after$predictions$.pred_class)` of `r nrow(test_data)` test rows. Both calibrated probabilities sit on the same side of 0.5 as the raw 0 and 1.
...
- ROC AUC is unchanged for both models here. This calibrator is a function of the predicted probability only, so it does not reorder rows. If I only need a ranking, I skip it.
...
[orbital](https://orbital.tidymodels.org/articles/supported-models.html){target="_blank"} 0.7.0 translates this `step_normalize()` recipe and this `decision_tree(engine = "rpart")` model to SQL, including class and probability predictions. `adjust_probability_calibration()` is outside that list (orbital's tailor support is `adjust_probability_threshold()`, `adjust_equivocal_zone()`, `adjust_numeric_range()`, and `adjust_predictions_custom()`), so this calibrated workflow stays in R.
